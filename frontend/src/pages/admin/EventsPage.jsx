...
const EventsPage = () => {
  ...
  const { events, agents, students, deleteEvent, notifyAgents } = useData();
  ...
  // KPI Calculations
  const kpis = useMemo(() => {
    const live = events.filter(e => getEventStatus(e) === 'live').length;
    const upcoming = events.filter(e => getEventStatus(e) === 'upcoming').length;
    const past = events.filter(e => getEventStatus(e) === 'completed').length;

    const totalSeats = events.reduce((acc, curr) => acc + (curr.seatCapacity || 0), 0);
    const totalFilled = students.length;
    const fillRate = totalSeats > 0 ? Math.round((totalFilled / totalSeats) * 100) : 0;

    return { live, upcoming, past, fillRate, total: events.length };
  }, [events, students]);
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(event => {
      ...
      // Country filter placeholder logic
      const matchesCountry = countryFilter === 'all' || (event.location && event.location.toLowerCase().includes(countryFilter.toLowerCase()));

      return matchesSearch && matchesType && matchesCountry;
    });
  }, [events, searchQuery, typeFilter, countryFilter]);

  // Dynamic Countries from events
  ...
  const getEnhancedEvent = (event) => {
    ...
    return {
      ...
      p,
      status,
      assignedAgentObjects: (event.assignedAgents || []).map(id => agents.find(a => String(a.id) === String(id))).filter(Boolean)
    };
  };
      ...
      >
        <div className="ev-card-inner">
          ...
          <div className="ev-meta">
            ...
            <div className="em-row">
              ...
              <span>{event.assignedAgents?.length || 0} agents assigned</span>
            </div>
          </div>
          ...
          <div className="seat-section">
            ...
            <div className="seat-lbl">
              <span>{enhanced.filled}/{enhanced.seats} seats</span>
        ...
        <div className="ev-card-bot">
          <div className="uni-chips">
            {/* Using mock unis as they are not in the current data structure */}
            {['Manchester', 'King\'s College', 'Leeds'].slice(0, 2).map((uni, idx) => (
              <div key={idx} className="uni-chip">{uni}</div>
            ))}
          </div>
  ...
  const renderSection = (title, status, countClass, divClass) => {
    const items = filteredEvents.filter(e => getEventStatus(e) === status);
      ...
      <div className="section-block mt-8">
        <div className="section-header">
          <div className="section-label-wrap">
            {status === 'live' && <div className="live-pulse" />}
            <span className="section-line-label">{title}</span>
            <span className={cn("section-count", countClass)}>
        ...
        <div className={cn("section-divider", divClass)} />
  ...
  return (
    <div className="pg min-h-screen bg-[#F9FAFB] font-['Inter']">
      <style>{`
        ...
        .sc-all { background: #f1f5f9; color: #475569; }
        ...
        .div-all { background: linear-gradient(90deg, #042C53 0%, #042C53 20%, transparent 100%); opacity: 0.8; }
        ...
        .detail-panel { 
          ...
          display: flex; flex-direction: column; animate: slideIn 0.3s ease-out;
        }
        ...
        .agent-item { display: flex; align-items: center; gap: 12px; padding: 12px; border: 1px solid #f1f5f9; border-radius: 10px; margin-bottom: 8px; }
        .agent-avatar { width: 32px; height: 32px; border-radius: 50%; background: #042C53; color: white; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 600; }
        ...
        .tab-btn-all.tab-btn-active { color: #042C53 !important; }
      ...
      <div className="toprow">
        <div>
          ...
          <p>All events — live, upcoming, and past</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button
            className="bg-[#042C53] hover:bg-[#0C447C] text-[#B5D4F4] text-xs font-bold h-9 px-5 rounded-lg transition-all"
      ...
      <div className="kpi-row">
        ...
        <div className="kpi">
          ...
          <div className="kpi-sub text-[#0C447C]">Next: {events.filter(e => isFuture(parseISO(e.date))).sort((a, b) => parseISO(a.date) - parseISO(b.date))[0]?.date || 'TBD'}</div>
        </div>
        <div className="kpi">
        ...
        <div className="kpi">
          <div className="kpi-label">Avg seat fill rate</div>
          <div className="kpi-val">{kpis.fillRate}%</div>
          <div className="kpi-sub text-[#791F1F]">Target: 85%</div>
        </div>
      </div>
      ...
      <div className="tab-filter-row">
        ...
        <div className="tab-strip">
          {[
            { key: 'all', label: 'All', countClass: 'sc-all', count: kpis.total },
      ...
      <div className={cn(
        'section-divider',
        activeTab === 'all' ? 'div-all' :
          activeTab === 'live' ? 'div-live' :
            activeTab === 'upcoming' ? 'div-up' :
              'div-past'
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Capacity</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-slate-500 uppercase tracking-wider">Agents</th>
                    ...
                    <tr key={event.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => openDetail(event)}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5">
                          <div className="text-xs font-bold text-slate-700">{enhanced.filled}/{enhanced.seats}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex -space-x-2">
                          {enhanced.assignedAgentObjects.slice(0, 3).map((agent, i) => (
                          ...
                          {enhanced.assignedAgentObjects.length > 3 && (
                          ...
                          {enhanced.assignedAgentObjects.length === 0 && <span className="text-xs text-slate-400 italic">None</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4">
          ...
          <div className="detail-panel">
            ...
            <div className="dp-body">
              <div className="stat-grid">
                ...
                <div className="stat-box">
                  <div className="stat-box-val">{selectedEvent.filled}</div>
                  <div className="stat-box-lbl">Filled</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-val">{selectedEvent.assignedAgents.length}</div>
                  <div className="stat-box-lbl">Agents</div>
                </div>
                <div className="stat-box">
                  <div className="stat-box-val">12%</div>
                  <div className="stat-box-lbl">Conversions</div>
                </div>
              </div>

              <div className="mb-8">
                <div className="flex justify-between items-end mb-2">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Seat Fill Rate</span>
              ...
              <div>
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Assigned Agents</h4>
                {selectedEvent.assignedAgentObjects.length > 0 ? (
                  <div className="space-y-2">
                    {selectedEvent.assignedAgentObjects.map(agent => (
                  ...
                  <div className="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <p className="text-[11px] font-medium text-slate-400">No agents assigned</p>
                  </div>
                )}
              </div>
            ...
            <div className="dp-footer">
              ...
              <Button
                ...
                onClick={() => notifyAgents(selectedEvent.id)}
              >
                Notify Agents
              </Button>
            </div>
